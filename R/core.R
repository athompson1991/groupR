...
groupr <- function(df, groups, functions = list("count" = "n()"), depth = length(groups)){
  ...
  group_level_list <- lapply(group_combinations, function(mtx){
    ...
    df <- apply(mtx, 2, function(column){
      ...
      worked_on_df <- dplyr::grouped_df(worked_on_df, group_selection)
      colnames(worked_on_df)[tail(1:ncol(worked_on_df), function_count)] <- column_names
      return(worked_on_df)
    })
  ...
  names(group_level_list) <- paste("n_", 0:depth, "_group", sep = "")

  group_level_list$meta <- list(groups = groups, functions = functions_raw)
...
gapply.groupr <- function(groupr, new_functions, is_cbind = F){
  ...
  core_groupr <- drop_grouping_level(groupr = groupr, group_level = length(groupr))
